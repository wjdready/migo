...
void lv_port_disp_init(void)
{
    ...
    EXT_RAM_ATTR static lv_color_t draw_buf_3_1[LV_HOR_RES_MAX * LV_VER_RES_MAX];            /*A screen sized buffer*/
    EXT_RAM_ATTR static lv_color_t draw_buf_3_2[LV_HOR_RES_MAX * LV_VER_RES_MAX];            /*An other screen sized buffer*/
    // lv_color_t *draw_buf_3_1 = malloc(LV_HOR_RES_MAX * LV_VER_RES_MAX*2);
    // lv_color_t *draw_buf_3_2 = malloc(LV_HOR_RES_MAX * LV_VER_RES_MAX*2);
    // if(draw_buf_3_1 == NULL || draw_buf_3_2 == NULL)
    // {
    //   abort();
    // }
    ...
    /*-----------------------------------
     * Register the display in LVGL#include "esp_system.h"
     *----------------------------------*/
...
static void disp_flush(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p)
{
    ...
    int16_t left = area->x1;
    int16_t top = area->y1;
    int16_t width = area->x2 - area->x1 + 1;
    int16_t height = area->y2 - area->y1 + 1;
    write_frame_rectangleLE(left, top, width, height, color_p);
